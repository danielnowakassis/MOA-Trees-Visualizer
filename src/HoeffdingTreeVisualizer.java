...
public class HoeffdingTreeVisualizer extends AbstractClassifier implements MultiClassClassifier,
CapabilitiesHandler {
   ...
   public static class Node extends AbstractMOAObject {
       ...
       public double[] getClassVotes(Instance inst, HoeffdingTreeVisualizer ht) {
           return this.observedClassDistribution.getArrayCopy();
       }
       ...
       public void describeSubtree(HoeffdingTreeVisualizer ht, StringBuilder out,
               int indent) {
       ...
       public void getGraphviz(HoeffdingTreeVisualizer ht, StringBuilder out, int index) {
           out.append(ht.index + " [label=\"" + ht.getClassNameString() + " = ");
           out.append(ht.getClassLabelString(this.observedClassDistribution.maxIndex()) + "\\n");
           out.append("weights: ");
           this.observedClassDistribution.getSingleLineDescription(out,
                   ht.treeRoot.observedClassDistribution.numValues());
           out.append("\", fillcolor=\"#ffffff\"] ;");
       }
   ...
   public static class SplitNode extends Node {
       ...
       public void getGraphviz(HoeffdingTreeVisualizer ht ,StringBuilder out, int index_anterior) {
           for (int branch = 0; branch < numChildren(); branch++) {	
               Node child = getChild(branch);
               if (child != null) {
                   ht.index += 1;
                   if((branch == 0 && numChildren() == 2) || numChildren() > 2) {
                       out.append((index_anterior) + " [label=\"" + this.splitTest.describeConditionForBranch(branch,
                               ht.getModelContext()) + "\"");
                       out.append(", fillcolor=\"#ffffff\"] ;");
                   }
                   
                   
                   //System.out.println(ht.index_2_anterior + " " + index_anterior);
                   
                   out.append(System.getProperty("line.separator"));
                   out.append(index_anterior + " -> " + (ht.index) + " ; ");
                   out.append(System.getProperty("line.separator"));

//					//num child = 2 & branch = 0
//					if(numChildren() == 2 && branch == 0) {}
//					//num child = 2 & branch = 1
//					else if(numChildren() == 2 && branch == 1) {}
                   if (child instanceof SplitNode) {
                       if (((SplitNode) child).getChildren().size() > 2) {
                           ht.index_2_anterior = index_anterior;
                       }
                   }
                   child.getGraphviz(ht, out, ht.index); 
               }
           }
       }
   ...
   public static abstract class LearningNode extends Node {
       ...
       public abstract void learnFromInstance(Instance inst, HoeffdingTreeVisualizer ht);
   }
   ...
   public static class InactiveLearningNode extends LearningNode {
       ...
       @Override
       public void learnFromInstance(Instance inst, HoeffdingTreeVisualizer ht) {
   ...
   public static class ActiveLearningNode extends LearningNode {
       ...
       public AttributeSplitSuggestion[] getBestSplitSuggestions(
               SplitCriterion criterion, HoeffdingTreeVisualizer ht) {
   ...
   public StringBuilder getDotRepresentation(StringBuilder out) {
       out.append("digraph {");
       out.append(System.getProperty("line.separator"));
       out.append("node [shape=box, style=\"filled\", color=\"black\", fontname=\"helvetica\"] ;");
       out.append(System.getProperty("line.separator"));	
       out.append("edge [fontname=\"helvetica\"] ;");
       out.append(System.getProperty("line.separator"));	
       this.treeRoot.getGraphviz(this, out, 0);
       out.append("}");
       return out;
   }
   ...
   protected void attemptToSplit(ActiveLearningNode node, SplitNode parent,
       ...
       if (!node.observedClassDistributionIsPure()) {
           ...
           if (bestSplitSuggestions.length < 2) {
               ...
               this.splitted = bestSplitSuggestions.length > 0;
           } else {
   ...
   public ImmutableCapabilities defineImmutableCapabilities() {
       if (this.getClass() == HoeffdingTreeVisualizer.class)
